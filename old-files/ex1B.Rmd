...
gamma_b2b <- function(alpha, n){
  ...
  for (i in 1:n){
    x1 = a*runif(1)
    x2 = b*runif(1)
    f_condition = f_gamma(x2/x1, alpha)^0.5
    if (0 <= x1 && x1 <= f_condition)  { 
      x[i] = x2/x1
  }
  } 
  return(x)
...
library(ggplot2)
samples_b2b <- gamma_b2b(alpha, n)
x <- seq(0,4,0.001)
data_b2b <- data.frame("samples_b2b"=samples_b2b)
ggplot(data=data_b2b, aes(data_b2b$samples_b2b)) +
geom_histogram(aes(y=..density..), breaks = seq(0, 4, by = 0.05)) +theme(plot.title = element_text(hjust = 0.5))+
xlim(c(0,4)) + labs(title="Samples from gamma distribution", x="x", y="Density") +
stat_function(fun = f_testb, args = list(alpha), color = "red")
