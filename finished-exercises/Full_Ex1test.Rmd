...
We know that the cumulative exponential distribution takes the form $$ F(x;\lambda) =  1 - \lambda e ^{-\lambda x},  \quad x \ge 0 
$$
...
```{r fig.cap = "Theoretical and simulated exponential distribution when lambda = 1."}
...
x = outval[[1]]
y = outval[[2]]
ggplot(data.frame(x=x))+geom_histogram(aes(x=x, y = ..density..), bins = 50)+geom_line(data = data.frame(x=x,y=dexp(x, rate = 1)), aes(x=x, y=y), color = "blue")+xlim(0,5)+labs(title="Exponential Distribution", x="x", y="Density")
```
...
```{r, fig.cap = "Simulated and theoretical values from our probability density function by the invesion method when alpha = 0.5."}
...
hist(x, breaks = 50, freq = FALSE, main = bquote("Histogram of samples from g when "~alpha == .(alpha)))
...
ggplot(data.frame(x = outval3[[1]], y = outval3[[2]]))+
  geom_histogram(aes(x=x, y=..density..),alpha=0.2)+geom_line(data = data.frame(x=x,y=checkfunc3(alpha,x)), aes(x=x, y=y), color = "blue")+labs(title="Probabiliy density function g(x)", x="x", y="Density",caption = "Theoretical and simulated values of our probability density function when alpha = 0.2.")

```
From the plot we see that our simulation follows the theoretical values. 
...
ggplot(std_normal_data)+
  geom_histogram(aes(x=x, y=..density..),bins=50)+geom_line(data = data.frame(x=x,y=dnorm(x)), aes(x=x, y=y), color = "blue")+labs(title="Standard normal distribution", x="x", y="Density",caption = "Theoretical and simulated values of the standard normal distribution.")
```

## Question A5
We want to sample from a d-variate normal distribution with mean $\mu$ and covariance matrix $\Sigma$. We use the function mynormal with the Box-Muller algorithm to construct a vector with values of $\mu$ and a positive definite matrix $\Sigma = MM^T$, where $M$ is a d by d matrix from a normal distribution. As we don’t have given values of $\mu$ and $\Sigma$, we just choose a $\mu$ and $\Sigma$ such that $\Sigma$ is positive definite by the Cholesky factorization. Note that in the code, $\Sigma = M^TM$ because of how the Cholesky function is implemented in R. 
...
xlim(c(0,4)) + labs(title="Samples from gamma distribution when alpha = 0.5", x="x", y="Density", caption = "Theoretical and simulated values from the gamma distribution function when alpha = 0.5.") +
...
$$
$$
b_{+} = \sqrt{{sup}_{x \ge 0} x^2 f^*(x)  } = \sqrt{(\alpha +1)^2
f^*(\alpha +1)}
$$
$$
...
\end{aligned}
$$
and to obtain the above result, we just use these values of $x$ to find the supremum. 
...
x_2 = b_+\cdot u_2
$$
which is a transform from the standard uniform variables $u_1, u_2$ to a uniform distribution between 0 and some other limit. In the log scale, this becomes
...
```{r, fig.cap = "Theoretical and simulated values of the gamma distribution when alpha = 400."}
n <- 10000
...
hist(samples_b2b, breaks = 40, freq = FALSE, main = bquote("Gamma distribution when "~alpha == .(alpha)))
...
```
```{r, fig.cap = "Number of tries for one accepted value in the simulation as function of alpha."}
plot(test_try[,1], test_try[,2], title(main = "Number of tries as function of alpha for ratio of uniforms method"), xlab = "Alpha", ylab = "Number of tries")
...
```{r, fig.cap = "Theoretical and simulated values of gamma distribution when alpha = 2, beta = 2"}
...
hist(samples_ab, breaks = 40, freq = FALSE, main = bquote("Gamma distribution when "~alpha == .(alpha) ~beta ==.(beta)))
...
```{r, fig.cap = "Theoretical and simulated values of the beta distribution when alpha = 0.5, beta = 0.5."}
...
hist(samples_beta, breaks = 40, freq = FALSE, main = bquote("Beta distribution when "~alpha == .(alpha) ~beta ==.(beta)))
...
xlim(c(0.4,0.85)) + labs(title="Posterior density", x="x", y="Density", caption = "Posterior density for theta given y.") +
...
geom_histogram(aes(y=..density..), breaks = seq(0, 1, by = 0.005)) +theme(plot.title = element_text(hjust = 0.5))+
xlim(c(0.4,0.85)) + labs(title="Posterior density with posterior mean", x="x", y="Density", caption = "Posterior density given y.") +
...
## Question D4
We want to use the Beta(1,5) distribution as the prior, instead of the U(0,1) distribution and importance sampling with self-normalizing weights to estimate the posterior mean. The new prior with a Beta(1,5) distribution is: 
$$
...
$$
which is biased, but for $n\to\infty$ it becomes approximately unbiased. 
...
```
The simulated posterior mean is `r simulated_d4`, and the numerical value is `r num_d4`. We see that they are equal.